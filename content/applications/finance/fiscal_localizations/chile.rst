...
Clients from Chile will be able to select if they need an invoice or a ballot for their purchase
with an extra step added during the checkout process.
...
be filled out, including the :guilabel:`Activity Description`, the :guilabel:`Identification Number`
...
application on the main Odoo dashboard, and search for the module by its technical name
...
- :guilabel:`Use QR code on ticket`: this feature enables a QR code to be printed on the user's
  receipt so that they can easily request an invoice after their purchase.
